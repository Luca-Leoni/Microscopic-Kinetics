...
\sol{Analytic solution}
{
    ...
    \end{equation}
    This condition for the minimum already shows how solutions with $\eta \neq 0$ are possible only for $\Omega < 0$, so ordered systems exist only for negative interaction parameter as according to \tabref{tab:phaseBinary}. Still, it's interesting to see how from this result we can expand near $\eta = 0$ to obtain the temperature were the transition from ordered to disordered appear in the case of $\Omega < 0$, given by
    \begin{equation}
